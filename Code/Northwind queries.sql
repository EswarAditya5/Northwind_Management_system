...
select  Country,CategoryName,round(sum(sales),2) as TotalSales from `order details` 
natural join orders
natural join customers
natural join products
natural join categories
...
select  CategoryName ,Country,round(sum(sales),2) as TotalSales from `order details` 
natural join orders
natural join customers
natural join products
natural join categories
...
# Employee wise Count of Orders
SELECT CONCAT(e.FirstName, ' ', e.LastName) AS EmployeeName, COUNT(*) AS OrderCount
FROM Orders o
...
# CountryWise CategoryName wise Count of Orders
SELECT o.ShipCountry AS Country, c.CategoryName, COUNT(*) AS OrderCount
FROM Orders o
JOIN `order details` od ON o.OrderID = od.OrderID
JOIN Products p ON od.ProductID = p.ProductID
JOIN Categories c ON p.CategoryID = c.CategoryID
